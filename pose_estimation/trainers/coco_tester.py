...
class CocoTester(Tester):
    ...
    def _init(self, config, normalization_method=None):
        ...
        self._test_image = np.array([test_image] * self._batch_size).reshape(self._batch_size, *im_shape).astype(np.float32)
    ...
    def evaluate(self, model, iteration):
        ...
        self.write_summaries(
            summaries={
                CocoTester.TEST_IMAGE: (heatmap[:1][..., :1] * 255.0).astype(np.uint8),
