...
class InputGenLayerV2Batched(GenLayer):
    def __init__(
        ...
        self.shuffle = shuffle
        self.buffer_size = buffer_size
        self.iterator = self.build_iterator(tfr_path_generator, operation_before_batched, map_operation, num_parallel_calls)
        super().__init__(
            name=name,
            input_tensor=self.iterator[input_data_type]
        )

    def build_iterator(self, gen: TFRPathGenerator, operation_before_batched, map_operation: TFRMapMethod, num_parallel_calls):
        tf_records = tf.data.Dataset.from_generator(
            gen.next_element,
            output_types={
                TFRPathGenerator.TFRECORD: tf.string
